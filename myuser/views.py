...
from .models import Estudiante, Docente, Curso, Matricula, AuditoriaMatricula
...
def home(request):
	...
	cursos = Curso.objects.select_related('docente').all()[:10]
	matriculas = Matricula.objects.select_related('estudiante', 'curso').all()[:10]
	auditorias = AuditoriaMatricula.objects.all()[:10]
	resumen_cursos = Curso.objects.annotate(total_matriculas=Count('matriculas')).order_by('codigo')[:10]
	promedio_notas = Matricula.objects.exclude(nota_final__isnull=True).aggregate(promedio=Avg('nota_final'))['promedio']
	context = {
		'all_users': all_users,
		...
		'matriculas_count': Matricula.objects.count(),
...
def user_signup(request):
	if request.method == 'POST':
		...
		user_model = get_user_model()
		user_obj = user_model.objects.create_user(email=email, name=name, password=password)
